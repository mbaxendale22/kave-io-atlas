...
const handleChange = e => {
    if (e.target.name === 'note1' || e.target.name === 'note2' || e.target.name === 'note3') {
...
const handleSubmit = async e => {
  ...
  return (
    ...
    <div className='border-2 border-black p-5 h-5/6 w-full grid grid-cols-4 grid-rows-1 gap-5'>
      <form onSubmit={handleSubmit} className=' flex-c-c border-2 border-gray-500 col-start-1 col-end-3 bg-gray-200 shadow-md'>
        ...
        <textarea className='form-input' type='text' name='description'></textarea>
        <button className='mt-2 form-input w-1/2'>Submit</button>
      </form>
      <form onSubmit={handleSubmit} className='border-2 border-black col-start-3 col-end-5 bg-gray-200 shadow-md flex-c-c'>
